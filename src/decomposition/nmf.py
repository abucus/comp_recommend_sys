...
class NMF(object):
    ...
    def factorize(self, V, WInit=None, HInit=None, max_iter=None):
        ...
        for iter_count in range(max_iter):
            ...
            W = self.__compute_argmin_matrix_for_f_wh(H=H)
            H = self.__compute_argmin_matrix_for_f_wh(W=W)
        return (W, H)
        
    def __compute_argmin_matrix_for_f_wh(self, W=None, H=None):
        '''
        ...
        for i in range(V.shape[1]):
            v = V[:, i]
            cols.append(self.__compute_argmin_x_for_fx(v, M))
        if H is not None:
    ...
    def __compute_argmin_x_for_fx(self, v, M, beta=.1,
        ...
        if self.__decrease_condition(v, M, x_old, x_new, grad_f_old):
            # increase step loop
            while iter_count < max_iter and self.__decrease_condition(v, M, x_old, x_new, grad_f_old):
